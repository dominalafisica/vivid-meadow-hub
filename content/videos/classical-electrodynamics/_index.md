---
title: "Classical Electrodynamics"
type: landing

sections:
  - block: markdown
    content:
      title: "Classical Electrodynamics"
      subtitle: ""
      text: |
        [⬅ Back to Video Lectures](/videos/)

        ---

        <div class="expand-container">
        <div class="video-grid">
        <div class="video-wrapper">
        {{< youtube "XPGWHCRUaUg" >}}
        </div>
        <div>
        <h3 style="margin-top: 0;">Electric Potential & Field of a Charged Sphere | Spherical Harmonics</h3>
        <p><strong>Description:</strong><br>
        <em>Calculation of the electric potential and field for a charged spherical surface using the powerful method of spherical harmonics expansion.</em></p>
        <details>
        <summary><span style="color:#3b82f6; cursor:pointer; font-weight:bold;">Read full description...</span></summary>
        <div style="margin-top: 10px; font-style: italic; font-size: 0.95rem;">
        Using this expansion it is remarkably simple to find the electrostatic potential in all space, then via the negative gradient it follows directly the electric field.
        </div>
        </details>
        </div>
        </div>
        </div>

        <hr style="opacity: 0.1; margin: 4rem 0;">

        <div class="expand-container">
        <div class="video-grid">
        <div class="video-wrapper">
        {{< youtube "aTIrFcpTb5Q" >}}
        </div>
        <div>
        <h3 style="margin-top: 0;">Magnetic Field of a Circular Loop | Biot-Savart Law</h3>
        <p><strong>Description:</strong><br>
        <em>Derivation of the magnetic field produced by a current-carrying circular loop along the z-axis using cylindrical coordinates and vector integration.</em></p>
        <details>
        <summary><span style="color:#3b82f6; cursor:pointer; font-weight:bold;">Read full description...</span></summary>
        <div style="margin-top: 10px; font-style: italic; font-size: 0.95rem;">
        In this video, we derive the magnetic field generated by a current-carrying circular loop along its symmetry axis ($z$-axis). By employing cylindrical coordinates and the Biot-Savart law, we demonstrate the systematic vector integration process. Additionally, we discuss the behavior of unit vectors in curvilinear coordinates and the fundamental rules governing the field's symmetry.
        </div>
        </details>
        </div>
        </div>
        </div>

        <hr style="opacity: 0.1; margin: 4rem 0;">
        

    design:
      spacing:
        padding: ['40px', '0', '80px', '0']
---
